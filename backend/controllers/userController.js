...
exports.getUsersWithVideos = async (req, res, next) => {
  try {
    const userIds = await videoRepository.GetDistinctUsersWithVideo();
...
exports.getUserFollowing = async (req, res, next) => {
  try {
    ...
    const userFollowings = await followRepository.UserFollowings(userId);

    const followings = userFollowings.map((follow) => ({
      profilePictureUrl: follow.followingId.profilePictureUrl,
