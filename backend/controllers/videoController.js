...
exports.postDeleteVideo = async (req, res, next) => {
  try {
    ...
    if (req.userId != video.user) {
      throw customError(ResponseMessages.NotUserVideo, 401);
    }

    if (await videoRepository.DeleteVideo(videoId)) {
      await viewedVideoRepository.DeleteVideoViews(videoId);
      await likeRepository.DeleteVideoLikes(videoId);
      await commentRespository.DeleteVideoComments(videoId);
    }

    res.status(200).send({ msg: ResponseMessages.VideoDeleted });
  } catch (error) {
    next(error);
  }
};
...
exports.getSearchVideos = async (req, res, next) => {
  try {
    ...
    const videos = await videoRepository.SearchVideos(searchQuery);
    const videosDto = videos.map((v) => ({
      id: v.id,
      title: v.title,
      thumbnail: v.thumbnailUrl,
...
exports.getSearchVideosByCategories = async (req, res, next) => {
  try {
    const catgory = req.query.category;

    const videos = await videoRepository.SearchVideosByCategory(catgory);
    const videosDto = videos.map((v) => ({
      id: v.id,
      title: v.title,
      thumbnail: v.thumbnailUrl,
